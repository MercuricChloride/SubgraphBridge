...
const SUBGRAPH_BRIDGE_CONTRACT_ADDRESS = "0xf5c4a909455C00B99A90d93b48736F3196DB5621";

const BRIDGE_QUERY_BLOCKS_BACK = 10;

const DEFAULT_QUERY_TEMPLATE_STRING = "{badgeWinner(block:{hash:\"\"},id:\"\",first:){votingPower}}";
const DEFAULT_QUERY_VARIABLE_VALUES = ["0xf412716874ddcd23d81e2d94048e48c0ad965522", "1"];
const DEFAULT_QUERY_BRIDGE: QueryBridge = {
    queryTemplate: "0x8c53762d54c02a8b9209b4d54ff280b24f6eb60b261833afba5783ace5e8c3ce",        // keccack256(DEFAULT_QUERY_TEMPLATE_STRING)
...
task("testStringToUint", "tests SubgraphBridge._strToUint")
.setAction(async (taskArgs, hre) => {
...
async function readDataStream(hre: HardhatRuntimeEnvironment) {
    ...
    const requestCID = await subgraphBridgeContract.test_requestCIDs(0);
    const d = await subgraphBridgeContract.dataStreams(defaultQueryBridgeID(hre), requestCID);
...
async function bridgeQuery(hre: HardhatRuntimeEnvironment) {
    ...
    let bQuery = DEFAULT_QUERY_TEMPLATE_STRING.substring(0, DEFAULT_QUERY_BRIDGE.blockHashOffset) + queryBlock.hash;
    if (DEFAULT_QUERY_VARIABLE_VALUES[0] == "") {
        bQuery += DEFAULT_QUERY_TEMPLATE_STRING.substring(DEFAULT_QUERY_BRIDGE.blockHashOffset + 66);
    } else {
        const qv0Idx = (DEFAULT_QUERY_BRIDGE.queryVariables[0] >> 8) - 66;
        bQuery += DEFAULT_QUERY_TEMPLATE_STRING.substring(DEFAULT_QUERY_BRIDGE.blockHashOffset, qv0Idx) + DEFAULT_QUERY_VARIABLE_VALUES[0];
        if (DEFAULT_QUERY_VARIABLE_VALUES[1] == "") {
            ...
            bQuery += DEFAULT_QUERY_TEMPLATE_STRING.substring(qv0Idx, qv1Idx) + DEFAULT_QUERY_VARIABLE_VALUES[1] + DEFAULT_QUERY_TEMPLATE_STRING.substring(qv1Idx);
        }
    }
